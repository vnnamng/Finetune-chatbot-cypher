...
7. What countries does Adobe operate in and what is their headcount for that country

```
MATCH (c:Company) - [r: OPERATES_IN] - (cn:Country) WHERE ANY (name IN c.names WHERE name = 'Adobe') RETURN cn, r
```

8. What countries does Apple operate in and what is the corporate tax rate in that country?
...
15. what are all the companies that will affect APPLE what are their relationships?

```
MATCH (c:Company) <-[r:COMPETES_WITH | SUPPLIES_TO | SUBSIDIARY_OF]- (a:Company) WHERE ANY(name IN a.names WHERE name = 'Apple') RETURN c, r UNION MATCH (c:Company) -[r:COMPETES_WITH | SUPPLIES_TO | SUBSIDIARY_OF]-> (a:Company) WHERE ANY(name IN a.names WHERE name = 'Apple') RETURN c, r
```
...
19. What is the parent company of Booking Holdings Inc

```
MATCH (c:Company) -[r:SUBSIDIARY_OF]-> (a:Company) WHERE ANY(name IN c.names WHERE name = 'Booking Holdings Inc') RETURN a
```
